#!/usr/bin/env node
// QVAC Tarot Card Meaning Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29360;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/tarot") {
      try {
        ...
        const result = await generate(modelId, card.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Tarot Card Meaning Generator GUI ready at http://localhost:${PORT}`);
  });
